...
void depalletizing_callback(const sensor_msgs::PointCloud2ConstPtr& pointcloud2_msg)
{
    ...
    // get raw data
    dataNode.FromMessagePointCloud2ForDepalletizer(*pointcloud2_msg);
    ...
    dataNode.ToHeightmapMsgsForDepalletizer(depalletizing_mapping_frame_id, depalletizing_mapping_msgs);
    ...
    pubPointCloud.publish(output_pointcloud);
...
int main(int argc, char** argv)
{
    ...
    pubDepalletizingMap = pc_nh.advertise<depalletizing_mapping_msgs::DepalletizingMap>("heightmap_msgs", 10);
    pubPointCloud = pc_nh.advertise<sensor_msgs::PointCloud>("depalletizing_msgs", 10);
